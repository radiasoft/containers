...
ELEGANT_DOCS=/usr/share/doc/elegant
ELEGANT_LICENSE=$ELEGANT_DOCS/LICENSE
TMP_LICENSE=$(mktemp)

cat > $TMP_LICENSE << EOF
Copyright (c) 2002 University of Chicago. All rights reserved.

Elegant is distributed subject to the following license conditions:

 SOFTWARE LICENSE AGREEMENT
 Software: Elegant

 1. The "Software", below, refers to Elegant (in either source code, or
...
sudo bash << EOF
mkdir -p $ELEGANT_DOCS
mv $TMP_LICENSE $ELEGANT_LICENSE
chown -R root: $ELEGANT_LICENSE
chmod -v ugo+rx $ELEGANT_DOCS
chmod -v ugo+r $ELEGANT_LICENSE
EOF

unset ELEGANT_DOCS
unset ELEGANT_LICENSE
unset TMP_LICENSE
