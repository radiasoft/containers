...
install_elegant_license() {
    local elegant_docs=/usr/share/doc/elegant
    local elegant_license=$elegant_docs/LICENSE
   
    if [[ -d $elegant_docs ]]; then
        return
    fi

    sudo mkdir -p $elegant_docs
    sudo chmod ugo+rx $elegant_docs
    
    sudo dd of="$elegant_license" << 'EOF'
...
EOF
    ...
    sudo chmod ugo+r $elegant_license
}
